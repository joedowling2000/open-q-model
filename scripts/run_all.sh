...
run_one() {
  ...
  ( cd q-evaluation-harness && /home/joedowling/venvs/qeval/bin/qeval generate \
      ...
      -o "$ROOT/results/$name.jsonl" ) > "logs/gen-$name.log" 2>&1
  local gen_rc=$?
  ...
  ( cd q-evaluation-harness && /home/joedowling/venvs/qeval/bin/qeval execute \
      "$ROOT/results/$name.jsonl" q-humaneval \
      -o "$ROOT/results/$name-scored.json" ) > "logs/exec-$name.log" 2>&1 || {
    echo "EXECUTE_FAILED $name"; return 1; }

  python3 - "results/$name-scored.json" "$name" <<'PY'
import json, sys
